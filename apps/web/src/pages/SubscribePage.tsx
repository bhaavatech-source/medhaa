...
export function SubscribePage({ apiUrl }: { apiUrl: string }) {
  ...
  async function chooseIndividualPlan() {
    if (!consentChecked) {
      setError('Please confirm the parental consent checkbox before continuing.');
      return;
    }
              ...
              {selectedIndividualPlan && (
                <div style={{ marginTop: 18, padding: 22, borderRadius: 22, background: 'linear-gradient(135deg,#eef2ff,#f5f3ff)', border: '1px solid #d9ddff' }}>
                  ...
                  <button
                    ...
                    style={{ width: '100%', marginTop: 8, padding: 14, borderRadius: 999, border: 'none', background: 'linear-gradient(90deg,#6366f1,#a855f7)', color: '#fff', fontWeight: 900, fontSize: 15, cursor: consentChecked ? 'pointer' : 'not-allowed', opacity: consentChecked ? 1 : 0.6 }}
                  >
                    Continue to Payment
                  </button>
                </div>
