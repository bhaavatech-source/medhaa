...
export class UIManager {
  ...
  renderPaletteItem(comp) {
    ...
    const statLine = [
      comp.thrust ? `${comp.thrust}g thrust` : null,
      comp.weight ? `${comp.weight}g` : null,
      comp.cells ? `${comp.cells}S` : null,
      comp.maxCells ? `Max ${comp.maxCells}S` : null
    ].filter(Boolean).join(" · ");
    el.innerHTML = `
  ...
  fillSlot(slotEl, comp) {
    ...
    const statLine = [
      comp.thrust ? `${comp.thrust}g` : null,
      comp.weight ? `${comp.weight}g` : null,
      comp.cells ? `${comp.cells}S` : null
    ].filter(Boolean).join(" · ");
    slotEl.innerHTML = `
