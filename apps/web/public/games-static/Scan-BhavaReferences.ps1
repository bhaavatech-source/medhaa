...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Scan-BhavaReferences.ps1
  powershell -ExecutionPolicy Bypass -File .\Scan-BhavaReferences.ps1 -RootPath "E:\medhaa\apps\web\public\games-static"
#>
...
# Get every file, then filter by extension AND exclude build/dependency folders.
...
if ($allFiles.Count -eq 0) {
    Write-Host "No matching files found under: $RootPath" -ForegroundColor Yellow
    Write-Host "Check that -RootPath points to the right folder and that it contains .html/.js/.css/.json files." -ForegroundColor Yellow
    exit 0
}
...
# functional code even if the regex matched.
$codeHints = @(
...
if ($results.Count -gt 0) {
    ...
    Write-Host "No 'Bhava'/'Bhāva' matches found in any scanned file." -ForegroundColor Yellow
}
