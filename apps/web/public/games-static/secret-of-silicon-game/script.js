...
const ContentDB = {
  ...
  factorySteps: [
  { id:'sand', icon:'🏖️', label:'Raw Sand', cost:'Very low raw-material cost', detail:'Not every sand is suitable. Beach sand can look right, but chip makers need ultra-pure quartz sand rich in silicon dioxide (SiO2). It is the starting point, not the final material.' },
  { id:'purify', icon:'🧪', label:'Purification', cost:'High energy + chemical cost', detail:'Silicon must be refined again and again. The purity target is extreme — often 99.9999999% or better. This step needs huge furnaces, chemistry, electricity, and very careful filtering.' },
  { id:'crystal', icon:'💎', label:'Crystal Growth', cost:'Expensive precision equipment', detail:'A tiny seed crystal is pulled slowly from molten silicon to form one giant single-crystal ingot. If the crystal has defects, the whole wafer can lose quality.' },
  { id:'wafer', icon:'🥞', label:'Wafer Slicing', cost:'Precision cutting cost', detail:'The ingot is sliced into very thin wafers and polished mirror-smooth. Even a tiny scratch can ruin patterns later, so this step uses diamond tools and special polishing machines.' },
  { id:'litho', icon:'🔬', label:'Photolithography', cost:'One of the most expensive steps', detail:'A mask shines ultraviolet light to print tiny patterns. This step often costs a lot because the machines are incredibly precise and the patterns are smaller than dust grains.' },
  { id:'etch', icon:'⚡', label:'Etching', cost:'Chemical + plasma cost', detail:'Unwanted material is removed with chemicals or plasma. The chip is not carved like wood — it is patterned layer by layer, and each layer must line up exactly.' },
  { id:'test', icon:'🧭', label:'Testing', cost:'Testing cost for every wafer', detail:'Every chip is checked while still on the wafer. Some are perfect, some have small failures, and some are marked bad. Testing saves money by finding mistakes early.' },
  { id:'package', icon:'📦', label:'Packaging', cost:'Assembly and protection cost', detail:'Good chips are cut apart, connected to tiny wires or bumps, and sealed. Packaging protects the chip from heat, moisture, and damage — and also helps it talk to the outside world.' },
  { id:'finished', icon:'✅', label:'Finished Chip', cost:'Highest value output', detail:'The final chip contains billions of tiny transistors in a space smaller than your fingernail. The real magic is that the sand has become a carefully designed brain for a device.' },
],

  devices: [
  { id:'phone', label:'Smartphone', subtitle:'A tiny supercomputer in your hand', chips:['Processor (SoC)','Memory Chip','Camera Sensor','Power Management Chip','Modem Chip','Touch Controller','Audio Codec'] },
  { id:'laptop', label:'Laptop', subtitle:'A work machine that balances speed and battery life', chips:['CPU','GPU','RAM Modules','SSD Controller','Wi-Fi Chip','USB Controller','Display Controller'] },
  { id:'robot', label:'Robot', subtitle:'A machine that senses, thinks, and acts', chips:['Motor Controller Chip','Sensor Processor','Microcontroller','Battery Management IC','IMU Chip','Voice Processor'] },
  { id:'drone', label:'Drone', subtitle:'A flying device that must think quickly and stay light', chips:['Flight Controller Chip','GPS Chip','Camera Processor','Radio Transceiver','Stabilization Chip'] },
  { id:'car', label:'Electric Car', subtitle:'A moving computer with motors, sensors, and safety systems', chips:['Battery Management IC','Motor Inverter Chip','Infotainment SoC','Sensor Fusion Chip','ADAS Processor','Charging Controller'] },
],

  future: [
  { id:'chiplets', tag:'Architecture', name:'Chiplets', fact:'Instead of one giant chip, smaller chips are packed together like LEGO blocks. This can improve yield and flexibility.' },
  { id:'gan', tag:'Power Electronics', name:'Gallium Nitride (GaN)', fact:'A material that can switch power very efficiently and handle high voltage and heat.' },
  { id:'sic', tag:'Power Electronics', name:'Silicon Carbide (SiC)', fact:'Often used in fast chargers and electric vehicles because it handles heat and power well.' },
  { id:'graphene', tag:'Experimental', name:'Graphene', fact:'A single layer of carbon atoms. It is extremely strong and can conduct electricity very well.' },
  { id:'quantum', tag:'Next-Generation Computing', name:'Quantum Chips', fact:'Use quantum bits that can behave in unusual ways, opening new possibilities for special computations.' },
  { id:'photonic', tag:'Next-Generation Computing', name:'Photonic Chips', fact:'Use light instead of only electrons for some kinds of fast communication and computing.' },
],
...
class GameState {
  ...
  completeLevel(n){
    ...
    if(this.data.unlockedLevel <= n) this.data.unlockedLevel = Math.min(n+1, LevelMeta.length);
    this.save();
  }
...
class SoundSystem {
  ...
  beep({freq=440, type='sine', duration=0.12, gain=0.15, attack=0.005, release=0.08, when=0} = {}){
    if(!this.enabled) return;
    this.ensure();
    if(!this.ctx) return;
  ...
  tap(){
    this.beep({ freq: 620, type:'triangle', duration:0.05, gain:0.08, release:0.04 });
  }

  wrong(){
    this.beep({ freq: 220, type:'sawtooth', duration:0.08, gain:0.07, release:0.08 });
    this.beep({ freq: 180, type:'sawtooth', duration:0.10, gain:0.06, when:0.05, release:0.08 });
  }

  correct(){
    this.beep({ freq: 523.25, type:'triangle', duration:0.08, gain:0.10 });
    this.beep({ freq: 659.25, type:'triangle', duration:0.10, gain:0.11, when:0.07 });
    this.beep({ freq: 783.99, type:'triangle', duration:0.12, gain:0.11, when:0.15 });
  }

  unlockLevel(){
    this.beep({ freq: 523.25, type:'sine', duration:0.07, gain:0.08 });
    this.beep({ freq: 783.99, type:'sine', duration:0.09, gain:0.09, when:0.08 });
    this.beep({ freq: 1046.5, type:'sine', duration:0.12, gain:0.08, when:0.16 });
  }

  complete(){
    this.beep({ freq: 523.25, type:'triangle', duration:0.09, gain:0.11 });
    this.beep({ freq: 659.25, type:'triangle', duration:0.09, gain:0.11, when:0.10 });
    this.beep({ freq: 783.99, type:'triangle', duration:0.11, gain:0.12, when:0.20 });
    this.beep({ freq: 1046.5, type:'triangle', duration:0.18, gain:0.13, when:0.32 });
  }

  badge(){
    this.beep({ freq: 698.46, type:'sine', duration:0.08, gain:0.08 });
    this.beep({ freq: 880.00, type:'sine', duration:0.12, gain:0.09, when:0.09 });
    this.beep({ freq: 1174.66, type:'sine', duration:0.16, gain:0.10, when:0.20 });
  }

  siliconMagic(){
    this.beep({ freq: 700, type:'sine', duration:0.08, gain:0.08 });
    this.beep({ freq: 980, type:'sine', duration:0.12, gain:0.08, when:0.06 });
    this.beep({ freq: 1320, type:'sine', duration:0.18, gain:0.09, when:0.15 });
  }
  ...
  startAmbience(){
    ...
    this.ambienceTimer = setInterval(()=>{
      ...
      osc.type = 'sine';
      osc.frequency.setValueAtTime([196, 220, 261.63][Math.floor(Math.random()*3)], now);

      g.gain.setValueAtTime(0.0001, now);
      g.gain.linearRampToValueAtTime(0.018, now + 0.8);
...
class BadgeSystem {
  ...
  award(id){
    ...
    if(isNew){
      ...
      if(this.sound) this.sound.badge();
      this.renderPanel();
      this.updateCount();
    }
  }
...
const LevelMeta = [
  ...
  { n: 14, title: 'Logic Gate Lab', section: 'Digital Basics'},
...
class LevelRenderers {
  constructor(app){
    ...
    this.badge = app.badge;
    this.curiosity = app.curiosity;
    this.tooltips = app.tooltips;
    this.stage = app.stage;
    this.levelListEl = app.levelListEl;
  }


  /* ---------- LEVEL 1 : Technology Everywhere ---------- */
renderLevel1(container){
  const q = Utils.el('div','big-question', '📱 Today’s Technology');
  container.appendChild(q);

  const intro = Utils.el('div','reveal-panel show', `
    <h4>Tap the devices</h4>
    <p>Each smart machine is made from many materials. Explore them all, then find the hidden brain of modern technology.</p>
  `);
  container.appendChild(intro);

  const techMaterials = {
    phone: ['Glass', 'Aluminum',  'Plastic',  'Copper',  'Gold',  'Silicon'],
    laptop: ['Aluminum',  'Copper',  'Plastic',  'Battery materials',  'Gold',  'Silicon'],
  ...
  const questionPanel = Utils.el('div','reveal-panel', `
    <h4>Now think like a tech detective 🕵️</h4>
    <p>All these devices contain many materials. But which one is the main brain-like material used inside the chips that help them think, calculate, and control?</p>
  `);
  ...
  const optionGrid = Utils.el('div','grid grid-5');
  optionGrid.style.display = 'none';

  const options = [
    { id:'gold', label:'Gold', icon:'🟡', correct:false, msg:'Gold helps in connections, but it is not the main chip brain.' },
    { id:'plastic', label:'Plastic', icon:'🧴', correct:false, msg:'Plastic is useful for covers and parts, but not for the chip brain.' },
    { id:'silicon', label:'Silicon', icon:'✨', correct:true, msg:'Correct! Silicon is the key semiconductor used in most chips.' },
    { id:'glass', label:'Glass', icon:'🪟', correct:false, msg:'Glass is useful in screens and covers, but not the main chip brain.' },
    { id:'copper', label:'Copper', icon:'🟠', correct:false, msg:'Copper is excellent for wires, but not the main chip brain.' }
  ];
  ...
  const siliconReveal = Utils.el('div','silicon-celebration', `
    ...
    <div class="reveal-panel show">
      <h4>Silicon is special</h4>
      <p>Silicon is not the only material in a device, but it is the most important material inside the tiny chips that process information and control smart actions.</p>
    </div>
  `);
  ...
  options.forEach(opt=>{
    ...
    const choose = ()=>{
      ...
      if(opt.correct){
        ...
        siliconReveal.style.display = 'block';
        siliconReveal.classList.add('pop-in');
        this.showFeedback(container, 'good', '🎉 Correct! Silicon is the key material used in most smart chips.');
        this.enableComplete(container, 1);
      }else{
        card.classList.add('wrong');
        this.toast.show(`❌ ${opt.msg}`);
      }
    };
...
/* ---------- LEVEL 4 : Conductors / Insulators / Semiconductors sorting ---------- */
  ...
  checkSortComplete(container, sortedCountEl){
    ...
    const counter = sortedCountEl ? sortedCountEl() : null;
    if(counter) counter.textContent = correctCount;
    if(allPlaced){
  ...
  view.innerHTML = '<div class="nucleus">Si<br>+14</div>';
  ...
  const drawAtom = (key)=>{
    ...
    el.shells.forEach((count, shellIdx)=>{
      ...
      if(!document.getElementById(`orbit-style-${shellIdx}`)){
        ...
        styleTag.textContent = `@keyframes orbit${shellIdx}{
    ...
    else if(key === 'oxygen') hint.textContent = 'Oxygen has 6 outer electrons. Try comparing it with carbon and silicon.';
    else if(key === 'sulfur') hint.textContent = 'Sulfur also has 6 outer electrons, which helps show patterns in the periodic table.';
    else if(key === 'aluminum') hint.textContent = 'Aluminium has 3 outer electrons, so it behaves differently from silicon.';
  ...
  const q = Utils.el('div','big-question', 'A transistor is a tiny semiconductor device used to amplify or switch electrical signals and power. It acts as the fundamental building block of modern electronics, regulating the flow of current or voltage. Billions of these miniature components are embedded into microchips and processors to power everything from smartphones to computers.🔀 A transistor is built from N-Type and P-Type silicon layers. Learn what those layers mean, then toggle the gate voltage and watch the switch work.');
  container.appendChild(q);

  const intro = Utils.el('div','reveal-panel show', `
    <h4>First, the idea</h4>
    <p><strong>N-Type</strong> silicon has extra free electrons that can move easily. <strong>P-Type</strong> silicon has holes, which act like missing electrons and can also move.</p>
    <p>When they meet, they form a <strong>PN junction</strong>. That junction is the heart of the transistor switch.</p>
  `);
  container.appendChild(intro);
  ...
  stage.innerHTML = `
    <div class="card switch-card">
      <p class="switch-title">N-Type</p>
      <div class="carrier-track ntrack"><span class="carrier-dot electron"></span><span class="carrier-dot electron"></span><span class="carrier-dot electron"></span><span class="carrier-dot electron"></span></div>
      <div class="switch-caption">Free electrons move here easily.</div>
    </div>
    <div class="card switch-card">
      <p class="switch-title">PN Junction</p>
      <div class="junction-bar"><span></span></div>
      <div class="switch-caption">A tiny barrier controls flow.</div>
    </div>
    <div class="card switch-card">
      <p class="switch-title">P-Type</p>
      <div class="carrier-track ptrack"><span class="carrier-dot hole"></span><span class="carrier-dot hole"></span><span class="carrier-dot hole"></span><span class="carrier-dot hole"></span></div>
      <div class="switch-caption">Holes move here like open spaces.</div>
    </div>
    <div class="card" style="text-align:center;">
      ...
      <div class="switch-caption" id="gateStateText">Gate OFF: no bridge across the junction.</div>
    </div>
    <div class="card" style="text-align:center;">
      <p style="margin-bottom:10px;">Current Flow</p>
      <div class="status-led" id="switchLed" style="margin:0 auto;"></div>
      <div class="switch-caption" id="currentStateText">Current is blocked.</div>
    </div>
  ...
  const gateStateText = Utils.qs('#gateStateText', container);
  const currentStateText = Utils.qs('#currentStateText', container);
  let toggles = 0;
  ...
  const toggle = ()=>{
    ...
    if(isOn){
      explain.textContent = 'The gate voltage opens a pathway across the junction. Electrons and holes can move — the transistor is ON.';
      gateStateText.textContent = 'Gate ON: the barrier is lowered.';
      currentStateText.textContent = 'Current flows through the channel.';
    } else {
      explain.textContent = 'No gate voltage means the pathway stays closed. The junction blocks current — the transistor is OFF.';
      gateStateText.textContent = 'Gate OFF: no bridge across the junction.';
      currentStateText.textContent = 'Current is blocked.';
    }

    if(toggles >= 2){
      this.badge.award('switch-master');
      this.showFeedback(container,'good','⚡ You just saw the core idea of a transistor: a tiny gate can control a much larger current. That is why computers can turn billions of switches on and off so quickly.');
  ...
  const q = Utils.el('div','big-question', '🏭 Click each stage in order to follow raw sand as it becomes a finished microchip. Each stage has a hidden cost, a reason, and a failure risk.');
  container.appendChild(q);

  const story = Utils.el('div','reveal-panel show', `
    <h4>Why this matters</h4>
    <p>Not every sand can become a chip. The process is expensive because purity must be extreme, machines must be ultra-precise, and a tiny mistake can damage thousands of chips at once.</p>
  `);
  container.appendChild(story);

  const progressBox = Utils.el('div','feedback show info', `Stages completed: <strong id="fabDone">0</strong> / ${ContentDB.factorySteps.length}`);
  container.appendChild(progressBox);

  const line = Utils.el('div','factory-line');
  const detailPanel = Utils.el('div','reveal-panel', `
    <h4 id="fabTitle">Tap a stage</h4>
    <p id="fabDetail">Each stage hides a question: Is the material pure enough? Is the machine precise enough? What could go wrong?</p>
    <p id="fabCost" style="margin-bottom:0;color:var(--accent-1);"></p>
  `);

  const doneEl = ()=>Utils.qs('#fabDone', container);
  const detailTitle = ()=>Utils.qs('#fabTitle', container);
  const detailText = ()=>Utils.qs('#fabDetail', container);
  const detailCost = ()=>Utils.qs('#fabCost', container);
  let nextIndex = 0;

  ContentDB.factorySteps.forEach((step, idx)=>{
    const card = Utils.el('div','card factory-step', `<span class="ficon">${step.icon}</span><div class="fnum">Step ${idx+1}</div><div>${step.label}</div><div style="margin-top:6px;font-size:11px;color:var(--text-2);">${step.cost || ''}</div>`);
    card.tabIndex = 0;
    card.setAttribute('role','button');
    ...
    card.addEventListener('click', ()=>{
      ...
      detailCost().textContent = step.cost ? `Typical cost pressure: ${step.cost}` : '';

      if(idx !== nextIndex){
        this.showFeedback(container,'bad', 'Try following the process in order — what has to happen before this step?');
        card.animate([{transform:'translateY(0)'},{transform:'translateY(-6px)'},{transform:'translateY(0)'}], {duration:280, easing:'ease-out'});
        return;
      }

      card.classList.add('done');
      nextIndex++;
      const counter = doneEl();
      if(counter) counter.textContent = nextIndex;
      this.showFeedback(container,'good', step.detail);

      if(nextIndex === 1) this.toast.show('Sand is only the beginning — purity matters a lot.');
      if(nextIndex === 2) this.toast.show('Purification is one of the biggest cost and energy steps.');
      if(nextIndex === 3) this.toast.show('A perfect crystal ingot is like a giant single family of atoms.');
      if(nextIndex === 5) this.toast.show('Photolithography is where the hidden patterns appear.');
    ...
    if(idx < ContentDB.factorySteps.length - 1){
      const arrow = Utils.el('div','factory-arrow', '➜');
...
renderLevel12(container){
  const q = Utils.el('div','big-question', '🚀 Beyond Silicon: what comes next after the materials and chips that changed the modern world?');
  container.appendChild(q);

  const intro = Utils.el('div','reveal-panel show', `
    <h4>Beyond Silicon</h4>
    <p>Silicon is still incredibly important, but engineers also explore new materials, new chip structures, and new computing ideas to solve harder problems.</p>
  `);
  container.appendChild(intro);
  ...
  ContentDB.future.forEach(f=>{
    ...
    card.addEventListener('click', ()=>{
      ...
      if(viewed.size === ContentDB.future.length){
        ...
        this.showFeedback(container,'good','🌟 You explored the ideas that go beyond silicon. The future may use new materials, new packaging, and new architectures together.');
      }
    });
    grid.appendChild(card);
  });
...
renderLevel14(container){
  const q = Utils.el('div','big-question', '🔌 Logic gates decide how chips think. Can you build the right output?');
  container.appendChild(q);

  const intro = Utils.el('div','reveal-panel show', `
    <h4>Logic Gate Lab</h4>
    <p>Logic gates are the basic building blocks of digital circuits. They combine simple signals to produce useful decisions.</p>
  `);
  container.appendChild(intro);

  const wrap = Utils.el('div','logic-lab');

  const truthCard = Utils.el('div','reveal-panel show', `
    <h4>Truth Rule</h4>
    <p>For this challenge, the output should turn ON only when both inputs are ON.</p>
  `);

  const controls = Utils.el('div','logic-controls');
  const aBtn = Utils.el('button','chip-btn', 'Input A: OFF');
  const bBtn = Utils.el('button','chip-btn', 'Input B: OFF');
  const checkBtn = Utils.el('button','chip-btn accent', 'Check Output');

  const gateBox = Utils.el('div','logic-gate-box');
  const gateLabel = Utils.el('div','logic-gate-label', 'AND Gate');
  const output = Utils.el('div','logic-output', 'Output: OFF');
  const status = Utils.el('div','feedback show info', 'Flip both inputs ON to activate the output.');
  const diagram = Utils.el('div','logic-diagram', `
    <div class="input-node" id="nodeA">A</div>
    <div class="input-node" id="nodeB">B</div>
    <div class="gate-node">AND</div>
    <div class="output-node" id="nodeOut">Y</div>
  `);

  gateBox.appendChild(gateLabel);
  gateBox.appendChild(diagram);
  gateBox.appendChild(output);

  wrap.appendChild(truthCard);
  ...
  const renderState = ()=>{
    aBtn.textContent = `Input A: ${a ? 'ON' : 'OFF'}`;
    bBtn.textContent = `Input B: ${b ? 'ON' : 'OFF'}`;
    aBtn.classList.toggle('on', a);
    bBtn.classList.toggle('on', b);

    const y = a && b;
    output.textContent = `Output: ${y ? 'ON' : 'OFF'}`;
    output.classList.toggle('on', y);

    Utils.qs('#nodeA', diagram).classList.toggle('on', a);
    Utils.qs('#nodeB', diagram).classList.toggle('on', b);
    Utils.qs('#nodeOut', diagram).classList.toggle('on', y);

    if(y && !completed){
      completed = true;
      this.enableComplete(container, 14);
      status.textContent = '✅ Correct! AND gate is ON only when both inputs are ON.';
      this.showFeedback(container, 'good', 'You built the correct logic behavior.');
    }else if(!completed){
      status.textContent = 'Flip both inputs ON to activate the output.';
    }
  };
  ...
  checkBtn.addEventListener('click', ()=>{
    const y = a && b;
    if(y){
      this.toast.show('✅ Correct output!');
      if(!completed){
        completed = true;
        this.enableComplete(container, 14);
        status.textContent = '✅ Correct! AND gate is ON only when both inputs are ON.';
      ...
      this.toast.show('❌ Not yet. Both inputs must be ON.');
      status.textContent = 'Try again: both inputs ON is the answer.';
    }
  });
  ...
  renderState();
  this.addCompleteButton(container, 14, false);
}


/* ---------- LEVEL 15 : Integrated Circuits ---------- */
renderLevel15(container){
  const q = Utils.el('div','big-question', '🧩 Millions of transistors and gates fit onto a single chip. Can you assemble the final circuit?');
  container.appendChild(q);

  const intro = Utils.el('div','reveal-panel show', `
    <h4>Integrated Circuits</h4>
    <p>An integrated circuit (IC) packs transistors, gates, and wiring onto one tiny piece of silicon. This is what makes modern electronics small, fast, and affordable.</p>
  `);
  container.appendChild(intro);

  const wrap = Utils.el('div','ic-lab');

  const stepCard = Utils.el('div','reveal-panel show', `
    <h4>Build the Chip</h4>
    <p>Assemble the chip in the correct order: place the transistors, connect the gates, then power the circuit.</p>
  `);

  const stepsRow = Utils.el('div','ic-steps');
  const stepButtons = [
    { id: 'transistors', label: '1. Place Transistors' },
    { id: 'gates', label: '2. Connect Gates' },
    { id: 'power', label: '3. Power Circuit' }
  ];

  const chipVisual = Utils.el('div','ic-chip', `
    <div class="ic-core">CHIP</div>
    <div class="ic-pins"></div>
  `);

  const status = Utils.el('div','feedback show info', 'Complete each step in order to power the chip.');

  wrap.appendChild(stepCard);
  wrap.appendChild(stepsRow);
  wrap.appendChild(chipVisual);
  wrap.appendChild(status);
  container.appendChild(wrap);

  let progress = { transistors:false, gates:false, power:false };
  let completed = false;

  const renderState = ()=>{
    chipVisual.classList.toggle('placed', progress.transistors);
    chipVisual.classList.toggle('wired', progress.gates);
    chipVisual.classList.toggle('powered', progress.power);

    if(progress.transistors && progress.gates && progress.power && !completed){
      completed = true;
      this.enableComplete(container, 15);
      status.textContent = '✅ Chip powered! You just modeled how a real integrated circuit comes together.';
      this.showFeedback(container, 'good', '🎉 You have completed The Secret of Silicon! From atoms to integrated circuits, you now understand how modern technology works.');
      this.badge.award('chip-hunter');
    }
  };

  stepButtons.forEach(step=>{
    const btn = Utils.el('button','chip-btn', step.label);
    btn.addEventListener('click', ()=>{
      const order = ['transistors','gates','power'];
      const idx = order.indexOf(step.id);
      const prevDone = idx === 0 || progress[order[idx-1]];

      if(!prevDone){
        this.toast.show('⚠️ Complete the previous step first.');
        return;
      }
      ...
      btn.disabled = true;
      status.textContent = `${step.label} done.`;
      renderState();
    });
    stepsRow.appendChild(btn);
  });

  this.addCompleteButton(container, 15, false);
}
  /* ---------- Shared helpers ---------- */
  addCompleteButton(container, levelNum, initiallyEnabled){
    ...
    btn.addEventListener('click', ()=>{
      if(btn.disabled) return;
            this.state.completeLevel(levelNum);
      if(this.app) {
        this.app.refreshNav();
        this.app.updateFooterButtons();
  ...
  showFeedback(container, type, message){
    let fb = Utils.qs('.feedback-toast', container);
    if(!fb){
      fb = Utils.el('div', 'feedback-toast feedback show');
      container.appendChild(fb);
    }
    fb.className = `feedback-toast feedback show ${type}`;
...
class App {
  constructor(){
    ...
    this.badge = new BadgeSystem(this.state, this.toast, this.sound);
    ...
    // Now instantiate renderers with App context
  ...
  updateFooterButtons(){
    ...
    if(prev) prev.disabled = this.currentLevel <= 1;
    if(next) next.disabled = this.currentLevel >= LevelMeta.length;
  }

  bindSoundToggle(){
    const host = Utils.qs('.topbar') || Utils.qs('header') || document.body;
    let btn = Utils.qs('#soundToggle');
    ...
    btn.addEventListener('click', ()=>{
      ...
      this.toast.show(enabled ? '🔊 Sound on' : '🔇 Sound off', 1400);
    });
  }

  bindBadgePanel(){
    const btn = Utils.qs('#badgeToggle');
    const panel = Utils.qs('#badgePanel');
    if(btn && panel){
      btn.addEventListener('click', ()=>{
  ...
  updateProgress(){
    ...
    const fill = Utils.qs('#progressFill');
    const label = Utils.qs('#progressLabel');
    if(fill) fill.style.width = pct + '%';
    if(label) label.textContent = `${completed} / ${LevelMeta.length} Levels`;
  }
}
