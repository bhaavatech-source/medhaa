Router.register('report', async (app)=>{
  ...
  // ── Bhāva Session End ──────────────────────────────────────────────────
  // rawScore: average of all 10 trait snapshot values (each 0–100)
  // BhavaSession.end() scales it to 60–160 internally: toScale(raw) = 60 + (raw/100)*100
  (function() {
    try {
      const avgTraitScore = Math.round(
        traits.reduce((sum, t) => sum + t.value, 0) / traits.length
      );
      const rawScore = Math.max(0, Math.min(100, avgTraitScore));
      if (typeof BhavaSession !== 'undefined' && BhavaSession.end) {
        BhavaSession.end(rawScore);
      }
    } catch(e) {
      // Bhāva session not available in this environment — silent no-op
    }
  })();
  // ──────────────────────────────────────────────────────────────────────
