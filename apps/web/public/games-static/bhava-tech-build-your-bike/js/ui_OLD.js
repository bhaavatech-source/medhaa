...
export class UIManager {
  ...
  renderBuild(bikeTypeId, levelId = "1") {
    ...
    build.levelDef.requiredSlots.forEach(slot => {
      ...
      const part = { id: `${slot}_generic`, name: slot.replace(/_/g," "), cost: 20, weightKg: 1 };
    ...
    if (build.levelDef.optionalSlots?.length) {
      ...
      build.levelDef.optionalSlots.forEach(slot => {
        ...
        const part = { id: `${slot}_generic`, name: slot.replace(/_/g," "), cost: 15, weightKg: 0.4 };
        card.innerHTML = `<span class="part-icon">${iconFor(slot)}</span><span class="part-name">${slot.replace(/_/g," ")} \u2728</span>`;
        this.dragDrop.makeDraggable(card, part.id, part, slot);
        tray.appendChild(card);
      });
  ...
  _handleDrop({ part, fromSlot, targetSlot, dropZoneEl }) {
    ...
    if (res.ok) {
      ...
      spawnParticles(document.getElementById("bike-canvas-wrap"), "spark", rect.width/2, rect.top, 6);
      const question = MENTOR_QUESTIONS[slot];
      this._mentorSay(question || `Nice! The ${slot.replace(/_/g," ")} is installed correctly.`);
    } else {
      dropZoneEl.classList.add("slot-error");
      statusEl.textContent = "\u26a0\ufe0f";
      audioBus.fail();
      this._mentorSay(`\u26a0\ufe0f ${res.errors[0]?.description || "Something doesn't fit here."} Why do you think that happens?`);
    }
    this._updateProgress();
  }
  ...
  _finishBuild(bikeTypeId) {
    ...
    inspector.innerHTML = `<h3>\ud83c\udfc6 Engineering Score: ${result.scoreResult.engineeringScore}/10</h3>
      ...
      <div style="margin-top:.8rem">${result.scoreResult.explanation.map(l => `<div class="alert info">${l}</div>`).join("")}</div>
      <button id="rebuild-btn" style="margin-top:1rem">Build Again</button>
    ...
    this._confetti(document.getElementById("bike-canvas-wrap"));
    audioBus.success();
  }
